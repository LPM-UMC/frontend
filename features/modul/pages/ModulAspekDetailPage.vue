...
async function loadDetail() {
  ...
  try {
    detailData.value = await aspekApi.getAspek(modulId.value, aspekId.value)
  } catch(e) {
    console.error(e)
  } finally {
...
<template>
  ...
  <section v-else-if="detailData" class="mx-auto w-full max-w-[1880px] bg-[#f4f4f4] px-3 pb-8 pt-5 sm:px-5 sm:pt-7 md:px-6 md:pt-8 lg:px-8 xl:px-10 2xl:px-12">
    <div class="flex items-center gap-3">
      <NuxtLink
        :to="`/dashboard/manajemen-modul/${modulId}`"
      ...
      <nav class="flex flex-wrap items-center gap-1">
        ...
        <span class="px-1 text-[clamp(0.95rem,1.2vw,1.05rem)] text-[#c5cad4]">/</span>
        <NuxtLink :to="`/dashboard/manajemen-modul/${modulId}`" class="text-[clamp(0.95rem,1.2vw,1.05rem)] text-[#9aa2b1] hover:text-[#6e7788]">Modul</NuxtLink>
        <span class="px-1 text-[clamp(0.95rem,1.2vw,1.05rem)] text-[#c5cad4]">/</span>
      ...
      <div class="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p class="mt-3 max-w-[1200px] text-[clamp(0.95rem,1.1vw,1.1rem)] leading-relaxed text-[#556173]">
            Gunakan halaman ini untuk melihat detail aspek evaluasi.
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="mt-6 space-y-6">
      <article class="rounded-[16px] border border-[#dadde3] bg-[#f4f4f5] shadow-[0_1px_4px_rgba(15,23,42,0.08)]">
        <button type="button" class="flex w-full items-center justify-between px-6 py-5 text-left" @click="toggleDetailSection('informasi')">
          <h2 class="text-[clamp(1.35rem,1.8vw,1.9rem)] font-semibold text-[#1e293b]">Informasi Aspek</h2>
        ...
        <div v-if="detailSectionOpen.informasi" class="border-t border-[#e3e7ee] px-6 py-6">
          <div class="grid gap-8 xl:grid-cols-2">
            <dl class="space-y-4">
              <div class="grid gap-1 sm:grid-cols-[260px_minmax(0,1fr)]">
                <dt class="text-[1.1rem] font-semibold text-[#5b6575]">Nama Aspek</dt>
          ...
          <div class="mt-6 flex flex-wrap gap-2 border-t border-[#e5e9f0] pt-4">
            <NuxtLink :to="`/dashboard/manajemen-modul/${modulId}/aspek/${aspekId}/edit`" class="rounded-[18px] bg-[#e30000] px-7 py-2 text-[0.95rem] font-semibold text-white shadow-[0_8px_16px_rgba(227,0,0,0.22)] transition hover:bg-[#ca0000]">
              Edit Aspek
            </NuxtLink>
            <button class="rounded-full border border-[#cfd5df] px-5 py-2 text-[0.95rem] font-semibold text-[#2b3340] transition hover:bg-[#f5f7fa]">
              Hapus
            </button>
          </div>
        </div>
