  ...
  <section class="mx-auto w-full max-w-360 px-3 pb-6 pt-4 sm:px-5 lg:px-8">
    ...
    <section
      ...
      <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <NuxtLink :to="localePath('/dashboard/manajemen-user/create')">
          <button
            class="inline-flex h-10 sm:h-11 min-w-40 items-center justify-center rounded-xl bg-[#e30000] px-5 text-sm sm:text-[0.95rem] font-semibold text-white shadow-[0_8px_18px_rgba(227,0,0,0.25)] transition hover:bg-[#c70000] cursor-pointer">
    ...
    <section
      ...
      <div class="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
        ...
        <article
          class="rounded-xl border border-[#e4e7ec] bg-[#f8f8f8] px-3 py-3 sm:px-4 sm:py-4 shadow-[0_4px_12px_rgba(15,23,42,0.08)]">
          <p class="text-xs sm:text-sm font-semibold text-[#44474d]">
            {{ $t('manajemenUser.totalUserAktif') }}
          </p>
        ...
        <article
          class="rounded-xl border border-[#e4e7ec] bg-[#f8f8f8] px-3 py-3 sm:px-4 sm:py-4 shadow-[0_4px_12px_rgba(15,23,42,0.08)]">
          <p class="text-xs sm:text-sm font-semibold text-[#44474d]">
            {{ $t('manajemenUser.totalRoleTerbanyak') }}
          </p>
    ...
    <section
      ...
      <div class="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <input v-model="searchQuery" type="search" :placeholder="$t('manajemenUser.cari')"
          class="h-10 w-full lg:w-72 rounded-xl border border-[#d8dde4] bg-[#f8f8f8] px-4 text-sm outline-none">

        <div class="flex flex-col gap-2 sm:flex-row">
          <select v-model="sortOrder"
            class="h-10 min-w-24 rounded-xl border border-[#d8dde4] bg-[#f8f8f8] px-4 text-sm">
            <option value="a-z">A-Z</option>
            <option value="z-a">Z-A</option>
          </select>

          <select v-model="roleFilter"
            class="h-10 min-w-32 rounded-xl border border-[#d8dde4] bg-[#f8f8f8] px-4 text-sm">
            <option disabled selected value="rolee">
        ...
        <table class="w-full min-w-220 text-sm">
          <thead>
            <tr class="bg-[#f1f3f6] text-[#2f3744]">
              <th class="px-3 py-3 text-left">{{ $t('manajemenUser.no') }}</th>
              <th class="px-3 py-3 text-left">{{ $t('manajemenUser.nama') }}</th>
              <th class="px-3 py-3 text-left">{{ $t('manajemenUser.email') }}</th>
              <th class="px-3 py-3 text-left">{{ $t('manajemenUser.role') }}</th>
              <th class="px-3 py-3 text-left">{{ $t('manajemenUser.status') }}</th>
              <th class="px-3 py-3 text-left">{{ $t('manajemenUser.tanggalBergabung') }}</th>
            </tr>
          </thead>

          <tbody>
            <tr v-for="(row, index) in paginatedRows" :key="row.id" class="cursor-pointer hover:bg-[#f7f9fc] transition"
      ...
      <div class="mt-4 flex justify-end">
        <div class="flex gap-2 text-sm">
          <button :disabled="currentPage === 1" class="rounded-xl border border-[#d8dde4] px-3 py-2 disabled:opacity-50"
            @click="currentPage--">
          ...
          <button :disabled="currentPage === totalPages"
            class="rounded-xl border border-[#d8dde4] px-3 py-2 disabled:opacity-50" @click="currentPage++">
