...
import {
  FM4_KAPRODI_PENDING_FILTER_OPTIONS,
  FM4_KAPRODI_SORT_OPTIONS,
  matchesFm4KaprodiPendingFilter,
  type Fm4KaprodiPendingDummyData,
  type Fm4KaprodiPendingFilterValue,
...
const pendingData = ref<Fm4KaprodiPendingDummyData | null>(null)
const loading = ref(true)

const searchKeyword = ref('')
const statusFilter = ref<Fm4KaprodiPendingFilterValue>('all')
...
const filteredRows = computed(() => {
  ...
  const rows = pendingData.value.rows.filter((row) => {
    ...
    const matchesStatus = matchesFm4KaprodiPendingFilter(row.status, statusFilter.value)
...
function handleAnalisisClick(temuanId: string) {
  navigateTo(
    `/dashboard/periode-modul/${encodeURIComponent(periodeModulId.value)}/unit/${encodeURIComponent(unitId.value)}/fm4/rtl/${encodeURIComponent(temuanId)}/create`
  )
}
...
watch(
  ...
  async (nextContext) => {
    loading.value = true
    pendingData.value = await repository.getPendingData(nextContext)
    loading.value = false
  },
  { immediate: true }
)
...
<template>
  <section class="mx-auto w-full max-w-[1720px] space-y-5 px-4 pb-8 pt-4 md:space-y-6 md:px-5 xl:px-6">
    <section v-if="pendingData" class="rounded-2xl border border-[#d5d8dd] bg-[#efefef] px-5 py-6 md:px-6 md:py-7">
    ...
    <section v-if="pendingData" class="rounded-2xl border border-[#d5d8dd] bg-[#efefef] px-5 py-6 md:px-6 md:py-7">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h2 class="text-[clamp(2.2rem,3.1vw,4rem)] font-semibold leading-tight text-[#11141b]">
          Daftar Temuan Belum Memiliki RTL
        </h2>
      ...
      <div class="mt-5 grid grid-cols-1 gap-3 xl:grid-cols-[1fr_auto_auto]">
        ...
        <label class="relative block w-full xl:w-[220px]">
          ...
          >
            <option
              v-for="option in FM4_KAPRODI_PENDING_FILTER_OPTIONS"
              :key="option.value"
              :value="option.value"
            >
              {{ option.label }}
            ...
            <tr v-for="row in paginatedRows" :key="row.id">
              ...
              <td class="border-b border-[#e8edf3] px-4 py-4 text-center">
                <button
                  ...
                  @click="handleAnalisisClick(row.id)"
                >
                  Analisis
                </button>
              </td>
