...
const progressData = ref<Fm4KaprodiProgressDummyData | null>(null)
...
const filteredRows = computed(() => {
  ...
  const rows = progressData.value.rows.filter((row) => {
    const searchable = `${row.title} ${row.rtl} ${row.aspect} ${row.createdAt}`.toLowerCase()
...
const summaryText = computed(() => {
  if (!progressData.value) return ''
  return `${progressData.value.summaryCount} ${progressData.value.summaryLabel}`
})

function handleProgressClick(rtlId: string) {
  navigateTo(
    `/dashboard/periode-modul/${encodeURIComponent(periodeModulId.value)}/unit/${encodeURIComponent(unitId.value)}/fm4/rtl/progres/${encodeURIComponent(rtlId)}/edit`
  )
}
...
watch(
  ...
  async (nextContext) => {
    loading.value = true
    progressData.value = await repository.getProgressData(nextContext)
    loading.value = false
  },
  { immediate: true }
)
...
<template>
  <section class="mx-auto w-full max-w-[1720px] space-y-5 px-4 pb-8 pt-4 md:space-y-6 md:px-5 xl:px-6">
    <section v-if="progressData" class="rounded-2xl border border-[#d5d8dd] bg-[#efefef] px-5 py-6 md:px-6 md:py-7">
    ...
    <section v-if="progressData" class="rounded-2xl border border-[#d5d8dd] bg-[#efefef] px-5 py-6 md:px-6 md:py-7">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h2 class="text-[clamp(2.2rem,3.1vw,4rem)] font-semibold leading-tight text-[#11141b]">
          Daftar RTL Belum Terlaksana
        </h2>

        <span class="inline-flex rounded-full bg-[#d1d5db] px-5 py-1.5 text-[clamp(1rem,1.2vw,1.4rem)] font-medium text-[#334155]">
          {{ summaryText }}
        </span>
      </div>
        ...
        <table class="w-full min-w-[1280px] border-separate border-spacing-0">
          <thead>
            <tr>
              ...
              <th class="border-b border-[#e3e7ee] bg-[#f1f3f6] px-4 py-3 text-left text-[clamp(1rem,1.1vw,1.25rem)] font-semibold text-[#2f3744]">RTL</th>
            ...
            <tr v-for="row in paginatedRows" :key="row.id">
              ...
              <td class="border-b border-[#e8edf3] px-4 py-4 text-center">
                <button
                  ...
                  @click="handleProgressClick(row.id)"
                >
                  Buat Progres
                </button>
              </td>
