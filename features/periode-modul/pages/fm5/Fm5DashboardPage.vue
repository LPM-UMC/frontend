...
const dashboardData = ref<Fm5DashboardDummyData | null>(null)
...
const filteredRows = computed(() => {
  ...
  return dashboardData.value.rows.filter((row) => {
    ...
    const searchable = `${row.programStudi} ${row.nomorBeritaAcara} ${row.tanggalMulai}`.toLowerCase()
...
function buildCreateRoute(): string {
  return `/dashboard/periode-modul/${encodeURIComponent(periodeModulId.value)}/unit/${encodeURIComponent(unitId.value)}/fm5/create`
}
  ...
  <section class="fm5-page mx-auto w-full max-w-[1600px] space-y-4 px-4 pb-7 pt-4 md:space-y-5 md:px-5 2xl:max-w-[1720px] xl:px-6">
    ...
    >
      <div class="grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
        ...
        <button
          v-if="dashboardData.hero.showCreateButton"
          type="button"
          ...
          <table class="w-full min-w-[1000px] border-separate border-spacing-0 xl:min-w-[1080px]">
            <thead>
              <tr>
                ...
                <th class="border-b border-[#e3e7ee] bg-[#f1f3f6] px-3 py-3 text-left text-[0.88rem] font-semibold text-[#2f3744] md:px-4 md:text-[0.94rem]">Nomor Berita Acara</th>
              ...
              <tr v-for="row in paginatedRows" :key="row.id">
                ...
                <td class="border-b border-[#e8edf3] px-3 py-3 text-center md:px-4 md:py-4">
                  <span
                    ...
                    :class="resolveFm5StatusMeta(row.status).badgeClass"
                  >
                    {{ resolveFm5StatusMeta(row.status).label }}
                  </span>
                </td>
                <td class="border-b border-[#e8edf3] px-3 py-3 text-center md:px-4 md:py-4">
                  <button
