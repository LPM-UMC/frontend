...
const pageTitle = computed(() => {
  if (false) return 'Edit Indikator Evaluasi'
...
const formTitle = computed(() => {
  if (false) return 'Form Edit Indikator Evaluasi'
...
const showQuestionField = computed(() =>
  false || form.type === 'scale'
)

const showScaleSection = computed(() => form.type === 'scale')

const showTypeField = computed(() => true)
...
watch(
  [() => 'create', lingkupId, objekId, indikatorId],
  () => {
    setupFormByMode()
  },
  { immediate: true }
...
<template>
  <section class="mx-auto w-full max-w-[1520px] px-4 pb-8 pt-6 sm:px-6 lg:px-8">
    <div class="flex items-center gap-3">
      <NuxtLink :to="backPath" class="inline-flex h-11 w-11 items-center justify-center rounded-full border border-[#d6dae2] bg-[#efeff1] text-[#596273] shadow-[0_2px_6px_rgba(15,23,42,0.08)] transition hover:bg-white" aria-label="Kembali">
