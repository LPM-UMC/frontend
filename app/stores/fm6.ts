...
export const useFm6Store = defineStore("fm6", {
  state: () => ({
  ...
  actions: {
    async fetchInformasi(pId: string, uId: string) {
      ...
      try {
        ...
        // FM6 uses the same informasi endpoint as FM1 for general periode-modul info
        ...
        this.error = err;
        console.error("Failed to fetch FM6 Informasi:", err);
      } finally {
