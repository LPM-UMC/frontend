...
// modulNameIds is removed since we use t('amiMenus.tindakLanjut')
...
onMounted(async () => {
  try {
    ...
    const modul = modulList.find((m) => m.nama === t('amiMenus.tindakLanjut'))
...
<template>
  <div class="px-4 py-6 sm:px-6 lg:px-8">
    <div class="mb-6">
      <h1 class="text-2xl font-bold tracking-tight text-slate-900">{{ t('amiDokumen.uploadDeskripsi', { modul: t('amiMenus.tindakLanjut', 'Tindak Lanjut') }) }}</h1>
    </div>
    ...
    <DokumenAmiView 
      v-else-if="modulId" 
      :modul-id="modulId" 
      :modul-name="t('amiMenus.tindakLanjut', 'Tindak Lanjut')" 
    />
  </div>
</template>
