  ...
  <div class="min-h-screen bg-gray-50 font-sans">
    ...
    <div class="bg-gradient-to-r from-[#e30613] to-[#b1040f] py-16 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center text-center shadow-inner">
      ...
      <p class="text-red-100 max-w-2xl text-sm md:text-base opacity-90">
        Ringkasan hasil Audit Mutu Internal per periode, standar, dan unit — dapat diakses publik
      </p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center text-center">
            <div class="text-3xl font-bold text-gray-900 mb-1">91%</div>
            <div class="text-xs text-gray-500 mb-2">Rata-rata capaian AMI</div>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center text-center">
            ...
            <div class="text-xs text-gray-500">Total unit diaudit</div>
          </div>
          <!-- Card 3 -->
          <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center text-center">
            <div class="text-3xl font-bold text-gray-900 mb-1">24</div>
            <div class="text-xs text-gray-500">Standar dievaluasi</div>
          </div>
          <!-- Card 4 -->
          <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center text-center">
            <div class="text-3xl font-bold text-gray-900 mb-1">12</div>
            <div class="text-xs text-gray-500">Auditor aktif</div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex bg-gray-100 rounded-full p-1">
              <button class="px-3 py-1 rounded-full text-xs font-medium bg-white shadow-sm text-[#e30613] border border-red-100">Standar</button>
              <button class="px-3 py-1 rounded-full text-xs font-medium text-gray-500 hover:text-gray-700">Unit</button>
              <button class="px-3 py-1 rounded-full text-xs font-medium text-gray-500 hover:text-gray-700">Auditor</button>
            </div>
          </div>
          <div class="h-64 relative w-full">
            <canvas id="trendChartAmi"></canvas>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div class="h-64 relative w-full flex items-center justify-center">
            <canvas id="distributionChartAmi"></canvas>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <!-- List Standar -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col">
          <h3 class="text-base font-bold text-gray-900 mb-4">Capaian per Standar (Top 4)</h3>
          <div class="space-y-0 flex-1">
            <div v-for="(std, index) in standarCapaian" :key="index" class="flex items-center justify-between py-4 border-b border-gray-100 last:border-0">
              <span class="text-sm font-medium text-gray-700">{{ std.name }}</span>
              <span :class="[
                'text-xs font-bold px-2 py-1 rounded',
                std.score >= 85 ? 'bg-green-50 text-green-700' :
                std.score >= 75 ? 'bg-yellow-50 text-yellow-700' :
                'bg-red-50 text-red-700'
              ]">{{ std.score }}%</span>
            </div>
          </div>
...
const unitCapaian = ref([
  { name: 'Fakultas Kesehatan', score: 96 },
  { name: 'Fakultas Teknik', score: 93 },
  { name: 'Fakultas Ekonomi', score: 87 },
  { name: 'Fakultas Hukum', score: 81 },
])

const standarCapaian = ref([
  { name: 'Standar Kompetensi Lulusan', score: 98 },
  { name: 'Standar Isi Pembelajaran', score: 92 },
  { name: 'Standar Proses Pembelajaran', score: 86 },
  { name: 'Standar Penilaian Pembelajaran', score: 88 },
])
...
const initTrendChart = () => {
  const canvas = document.getElementById('trendChartAmi') as HTMLCanvasElement
  if (!canvas) return
  ...
  trendChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Rata-rata Capaian AMI',
        data: [79, 81, 84, 88, 89, 91],
        borderColor: '#e30613',
...
const initDistributionChart = () => {
  const canvas = document.getElementById('distributionChartAmi') as HTMLCanvasElement
  if (!canvas) return
